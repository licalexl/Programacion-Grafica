...
public class Piramide4 : MonoBehaviour
{
    ...
    {
        yield return StartCoroutine(TranslateY(0.5f, 3.0f));
        yield return StartCoroutine(RotateY(45.0f, 3.0f));
    }

    // Método para trasladar la pirámide en el eje Y
    IEnumerator TranslateY(float amount, float duration)
    {
        Vector3 targetPosition = initialPosition + Vector3.up * amount;
        float elapsedTime = 0;
    ...
    // Método para rotar la pirámide en el eje Y
    IEnumerator RotateY(float targetAngle, float duration)
    {
